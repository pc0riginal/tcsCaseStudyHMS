...
class AddPatient(FlaskForm):
    ...
    name = StringField(label="Name",validators=[DataRequired(),Length(min=3,max=25),Regexp("^[a-zA-Z .'-]+$",message="only alphabets are allowed")])
    ...
    city = StringField('City',validators=[DataRequired(),Length(max=20),Regexp("^[a-zA-Z]+$",message="only alphabets are allowed")])
    state = StringField('State',validators=[DataRequired(),Length(max=20),Regexp("^[a-zA-Z]+$",message="only alphabets are allowed")])
